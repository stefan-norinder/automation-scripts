    ...
    public class FilesControlllerTests
    {
        ...
        public void GetRowWithQuotes()
        {
            ...
            fileServiceMock.Setup(x => x.GetFileNames(It.IsAny<string>(), It.IsAny<string>()))
                .Returns(new[] { "foo.txt" });
            fileServiceMock.Setup(x => x.Read(It.Is<string>(p => p == "foo.txt"), It.IsAny<bool>()))
                .Returns($"First row for now{Environment.NewLine}<row with=\"qouts\">{Environment.NewLine}This is the third line");
            var controller = new FilesController(fileServiceMock.Object);
            var result = controller.SearchInFiles(fileName, "*with=\"qouts\">*");
            Assert.AreEqual(1, result.First().Rows.Count());
            Assert.AreEqual("[2] <row with=\"qouts\">", result.First().Rows.First());
        }
